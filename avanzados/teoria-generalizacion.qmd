...
![La complejidad demasiado baja conduce a subajuste y la demasiado alta puede aumentar la brecha de generalización.](../estilos/figuras/generalizacion-complejidad.png){#fig-generalizacion-complejidad width=82% fig-alt="Curvas de riesgo empírico y riesgo esperado frente a la complejidad del modelo"}
...
## Ejemplo numérico de una cota de Hoeffding {.unnumbered}

Supóngase que un clasificador presenta un error empírico de \(0.12\) sobre \(n=500\) observaciones y se desea una garantía con \(\delta=0.05\). La desviación de Hoeffding es
...
y $\beta_n\to0$, la influencia de una observación disminuye con $n$. Los algoritmos regularizados suelen ser más estables. Esta relación fue formalizada, entre otros, por @bousquet2002stability.
...
El error de prueba puede disminuir, aumentar cerca del umbral de interpolación y volver a disminuir en el régimen sobreparametrizado. Este comportamiento se denomina doble descenso y ofrece una explicación parcial de la generalización en modelos altamente sobreparametrizados [@belkin2019reconciling].
...
## Seudocódigo formal para selección con validación anidada {.unnumbered}
...
3. Devolver
      \widehat R_{nested}
      = (1/K_out)\sum_{r=1}^{K_out}\widehat R_r.
```
:::
