# Redes neuronales\index{Redes neuronales} artificiales {#sec-redes-neuronales}

## Introducción {.unnumbered}

Las redes neuronales componen transformaciones afines y activaciones no lineales. Su entrenamiento moderno se apoya en retropropagación\index{Retropropagación}, optimización estocástica, inicialización controlada y regularización [@rumelhart1986learning; @glorot2010understanding; @he2015delving; @srivastava2014dropout\index{Dropout}].
...
## Interpretación de los pesos {.unnumbered}
...
## Frontera del perceptrón {.unnumbered}
...
## Regla de aprendizaje del perceptrón {.unnumbered}
...
## Limitación del perceptrón {.unnumbered}
...
::: {.callout-important title="Necesidad de no linealidad" appearance="default" icon="true"}
...
::: {.callout-note title="Analogía biológica y computacional" appearance="default" icon="true"}
Las dos figuras siguientes se presentan por separado para destacar con claridad la analogía entre la neurona biológica y la neurona artificial. En particular, en la neurona artificial todas las entradas del vector de características y el sesgo convergen explícitamente al nodo de suma ponderada.
:::

![Neurona biológica con sus partes principales: dendritas, soma, núcleo, axón, vainas de mielina y terminales axónicas.](../estilos/figuras/neurona-biologica.png){#fig-neurona-biologica width=86% fig-alt="Diagrama de neurona biológica"}

![Neurona artificial con entradas $x_1,\dots,x_5$ y sesgo, todas dirigidas al nodo de suma ponderada $z = \sum_{j=1}^{p} w_j x_j + b$, seguida de la función de activación y la salida $\hat y$.](../estilos/figuras/neurona-artificial.png){#fig-neurona-artificial width=82% fig-alt="Diagrama de neurona artificial"}

## Red multicapa {.unnumbered}
...
## Propagación hacia adelante {.unnumbered}
...
## Tangente hiperbólica {.unnumbered}
...
## Estabilidad numérica de softmax {.unnumbered}
...
## Entropía cruzada binaria {.unnumbered}
...
## Entropía cruzada multiclase {.unnumbered}
...
## Descenso estocástico {.unnumbered}
...
::: {.callout-note title="Retropropagación no es el optimizador" appearance="default" icon="true"}
...
![Una red neuronal multicapa transforma entradas en salidas mediante capas ocultas y activaciones no lineales.](../estilos/figuras/red-neuronal-arquitectura.png){#fig-red-neuronal-arquitectura width=84% fig-alt="Arquitectura con capa de entrada, dos ocultas y capa de salida"}

::: {.callout-note title="Lectura conceptual de la figura" appearance="default" icon="true"}
...
## Derivación para una neurona de salida {.unnumbered}
...
## Error local de salida {.unnumbered}
...
## Propagación hacia capas ocultas {.unnumbered}
...
## Gradientes respecto de pesos {.unnumbered}
...
## Gradientes respecto de sesgos {.unnumbered}
...
## Retropropagación completa {.unnumbered}
...
## Ejemplo de una red de una capa oculta {.unnumbered}
...
## Problema de gradiente desvaneciente {.unnumbered}
...
## Problema de gradiente explosivo {.unnumbered}
...
## Estrategias de estabilización {.unnumbered}
...
## Inicialización de pesos {.unnumbered}
...
## Inicialización Xavier {.unnumbered}
...
## Tasa de aprendizaje {.unnumbered}
...
::: {.callout-note title="Dropout durante entrenamiento y predicción" appearance="default" icon="true"}
...
## Interpretación de dropout {.unnumbered}
...
## Normalización por lotes {.unnumbered}
...
## Capacidad del modelo {.unnumbered}
...
::: {.callout-warning title="Alcance del teorema universal" appearance="default" icon="true"}
El teorema garantiza capacidad de aproximación bajo determinadas condiciones, pero no asegura entrenamiento eficiente, buena generalización ni una arquitectura pequeña [@hornik1989multilayer].
:::

La historia formal de las neuronas artificiales parte de @mcculloch1943logical y del perceptrón de @rosenblatt1958perceptron. Las arquitecturas profundas modernas se analizan en @goodfellow2016deep, mientras que @rumelhart1986learning explicó la retropropagación de manera operativa.

## Teorema de aproximación universal {.unnumbered}
...
## Profundidad frente a anchura {.unnumbered}
...
## Interpretación probabilística {.unnumbered}
...
## Relación con perceptrón {.unnumbered}
...
## Relación con aprendizaje profundo {.unnumbered}
...
## Ejemplo numérico de propagación {.unnumbered}
...
```text
Algoritmo Retropropagación con descenso por gradiente
Entrada:
  D = {(x_i, y_i)}_{i=1}^n, arquitectura de red, tasa η
Salida:
  parámetros Θ = {W^{(\ell)}, b^{(\ell)}}_{\ell=1}^L

1. Inicializar aleatoriamente Θ.
2. Repetir hasta convergencia:
   Para cada par (x_i, y_i):
   a) Propagación hacia adelante:
         a^{(0)} = x_i,
         z^{(\ell)} = W^{(\ell)} a^{(\ell-1)} + b^{(\ell)},
         a^{(\ell)} = φ^{(\ell)}(z^{(\ell)}),   \ell=1,...,L.
   b) Calcular la pérdida \mathcal{L}(y_i, a^{(L)}).
   c) Propagación hacia atrás:
         δ^{(L)} = ∇_{a^{(L)}}\mathcal{L} \odot φ'^{(L)}(z^{(L)}),
         δ^{(\ell)} = ((W^{(\ell+1)})^T δ^{(\ell+1)}) \odot φ'^{(\ell)}(z^{(\ell)}).
   d) Actualizar:
         W^{(\ell)} ← W^{(\ell)} - η δ^{(\ell)} (a^{(\ell-1)})^T,
         b^{(\ell)} ← b^{(\ell)} - η δ^{(\ell)}.
3. Devolver la red entrenada.
```
:::
...
## Ejercicio de neurona {.unnumbered}
...
## Ejercicio de dimensiones {.unnumbered}
...
## Ejercicio de retropropagación {.unnumbered}
...
Los fundamentos se apoyan en @mcculloch1943logical, @rosenblatt1958perceptron, @rumelhart1986learning, @hornik1989multilayer, @goodfellow2016deep, @glorot2010understanding, @he2015delving y @srivastava2014dropout.
