# Regresión logística\index{Regresión logística} {#sec-regresion-logistica}
...
2. interpretar probabilidades, odds\index{Odds} y logit;
...
::: {.callout-note title="Odds y logit" appearance="default" icon="true"}
...
::: {.callout-important title="Estimación por máxima verosimilitud" appearance="default" icon="true"}
...
::: {.callout-warning title="Separación completa" appearance="default" icon="true"}
...
La regresión logística transforma una combinación lineal de predictores en una probabilidad mediante la función logística. La decisión de clasificación se obtiene comparando dicha probabilidad con un umbral, usualmente $0.5$.

![Ilustración de la regresión logística mostrando la curva sigmoide, el umbral de decisión y la separación entre las clases 0 y 1.](../estilos/figuras/regresion-logistica-ilustracion.png){#fig-regresion-logistica width=82% fig-alt="Curva logística o sigmoide con probabilidad en el eje vertical y frontera de decisión en 0.5"}

A medida que la combinación lineal $\eta=\beta_0+\beta_1x$ aumenta, la probabilidad $P(Y=1\mid x)$ crece de forma suave desde valores cercanos a 0 hasta valores cercanos a 1.
...
Entrada:
  D = {(x_i, y_i)}_{i=1}^n, con y_i ∈ {0,1}
Salida:
  \hatβ y \hat p_i = P(Y_i=1|x_i)

1. Inicializar β^{(0)}.
2. Para t = 0,1,2,... hasta convergencia:
   a) Calcular
         η_i^{(t)} = x_i^T β^{(t)},
         p_i^{(t)} = 1/(1 + e^{-η_i^{(t)}}).
   b) Calcular el gradiente
         g^{(t)} = X^T (y - p^{(t)}).
   c) Calcular la Hessiana
         H^{(t)} = -X^T W^{(t)} X,
      donde W^{(t)} = diag(p_i^{(t)}(1-p_i^{(t)})).
   d) Actualizar por Newton-Raphson/IRLS:
         β^{(t+1)} = β^{(t)} - [H^{(t)}]^{-1} g^{(t)}.
3. Definir \hatβ = β^{(t+1)}.
4. Clasificar mediante
      \hat y(x)=1 si \hat p(x) ≥ c, y 0 en otro caso.
```
:::
